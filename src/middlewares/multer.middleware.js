...
const storage = multer.diskStorage({ // This method is used to configure storage settings for multer to store files on disk. You can alternatively use memoryStorage to store files in memory, but this is not recommended for large files
  // diskStorage is used to store the file on the disk storage of the server. MemoryStorage is used to store the file in the memory of the server which is not recommended for large files
